'use strict';
var learnjs = {
	poolId : 'us-east-1:15d898ce-1c0c-42fd-a92a-72ebb324d568'
};
...
learnjs.problemView = function(data) {
	...
	function checkAnswer() {
		var test = problemData.code.replace('__', answer.val())
				+ '; problem();';
		return eval(test);
	}
...
learnjs.fetchAnswer = function(problemId) {
	return learnjs.identity.then(function(identity) {
		...
		return learnjs.sendDbRequest(db.get(item), function() {
			return learnjs.saveAnswer(problemId);
		});
	});
...
learnjs.countAnswers = function(problemId) {
	return learnjs.identity.then(function(identity) {
		...
		var params = {
			TableName : 'learnjs',
			Select : 'COUNT',
			FilterExpression : 'problemId = :condition',
